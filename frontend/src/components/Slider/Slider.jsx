    ...
    return (
        <div className=''>
            ...
            <div className='flex align-center items-center justify-center'>
                <button className='bg-gray-darker bg-opacity-30 px-3 h-10 ' onClick={anteriorImagen}>↶</button>
                <div className='relative flex flex-row justify-between w-52 h-52 sm:w-96 sm:h-96 items-center overflow-hidden'>
                    {imagenes.map((imagen, index) => {
                        return <img 
                        key={index} 
                        src={imagen} 
                        className={`absolute transition-all duration-[2000ms] ${imagenActual === index ? ' left-0 opacity-100' : ' left-full opacity-0'}`}
                        alt="imagen" />
                    })}
                </div>
                <button className='bg-gray-darker bg-opacity-30 px-3 h-10 ' onClick={siguienteImagen}>↷</button>
            </div>
        </div>
    )
}
